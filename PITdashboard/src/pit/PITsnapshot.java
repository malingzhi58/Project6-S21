...
public class PITsnapshot extends HttpServlet {
    ...
    LinkedList<String> commodities = new LinkedList<>();

    @Override
    public void init() {
        // Add the commodities.
        // Each commodity should be unique and the number should equal numPlayers
        commodities.add("Platinum");
        commodities.add("Palladium");
        commodities.add("Iridium");
        commodities.add("Rhodium");
        commodities.add("Ruthenium");
    }
     ...
     */
    private void sendInitSnapshot(Context ctx, Session session) {
        try {
    ...
    private void resetAllPlayers(int numPlayers, int action) {
        ...
        try {
            ...
            for (int player = 0; player < numPlayers; player++) {
                ...
                if (!((m.getObject()) instanceof Reset)) {
